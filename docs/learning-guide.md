...
이 문서는 `my-webview-app`의 실제 source를 Codex와 대화하며 한 서브 스텝씩 학습하기 위한 durable 기준 자료다. 구현 범위는 [구현 계획서](./implementation-plan.md), 유지보수 관점의 전체 지도는 [내부 구조와 동작](./architecture-internals.md), 혼자 source를 따라가는 순서는 [소스 주석 읽기 안내서](./source-commentary-guide.md)를 사용한다.

이 학습서는 범용 React Native 교재가 아니다. 현재 repository에서 실제로 연결되는 provider, state, WebView, bridge, native service와 Query 흐름을 이해하는 데 필요한 개념만 설명한다.
...
3. 한 번에 하나의 서브 스텝만 다음 형식으로 대화창에 제시한다.
   ...
   - data·event·state 수명 예시
   - 사용자가 직접 확인할 지점
   - 핵심 요약
4. 사용자는 연결된 source를 직접 읽고 질문한다.
5. 질문 과정에서 설명 오류·누락이나 source 결함 후보가 나오면 근거를 다시 확인한다.
6. 확인된 설명 보완은 이 문서의 관련 단락에 기존 내용을 보존하며 추가한다.
7. source 동작 수정이 필요하면 학습 문서 수정과 섞지 않고 별도 Impact Review·승인·검증 범위로 분리한다.
...
필요한 문법과 증거 범위는 해당 source를 읽는 바로 그 단락에서만 짧게 설명한다.
