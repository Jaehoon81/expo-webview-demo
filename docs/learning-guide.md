...
6. 처음 보는 FLOW라면 소스 주석 읽기 안내서의 `실제 소스로 따라가는 FLOW-NN 핵심 경로`에서 대표 경로의 단계별 발췌와 화살표를 먼저 연결한다. 각 제목의 line link로 실제 source를 연 뒤 전체 단계 지도에서 제외된 `N-A`, `N-B` branch까지 확인하고, callback의 caller와 Promise 반환 consumer가 다음 단계에 이어지는지도 함께 말해 본다.
